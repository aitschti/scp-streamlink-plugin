...
class StripchatProxy(Plugin):
    ...
    def _start_proxy(self, host, port):
        ...
        proxy_script_path = os.path.join(plugin_dir, 'stripchat_proxy_v2.py')  # Update if path differs
        
        if not os.path.exists(proxy_script_path):
            self.logger.error(f"Proxy script not found at {proxy_script_path}. Please place 'stripchat_proxy_v2.py' in the same directory as the plugin.")
            return False
